...
echo -e "${NC}"
zelnodeprivkey="$(whiptail --title "ZelNode ANALIZER/FiXER v2.0" --inputbox "Enter your zelnode Private Key generated by your Zelcore/Zelmate wallet" 8 72 3>&1 1>&2 2>&3)"
zelnodeoutpoint="$(whiptail --title "ZelNode ANALIZER/FiXER v2.0" --inputbox "Enter your zelnode Output TX ID" 8 72 3>&1 1>&2 2>&3)"
zelnodeindex="$(whiptail --title "ZelNode ANALIZER/FiXER v2.0" --inputbox "Enter your zelnode Output Index" 8 60 3>&1 1>&2 2>&3)"
echo -e "${YELLOW}=====================================================${NC}"
...
then
        ...
        echo -e "${X_MARK} ${CYAN}IP was not detected try edit /etc/hosts and add there 'your_external_ip hostname' your hostname is $(hostname) ${RED}(only if zelback status is disconnected)${CYAN}" && sleep 1
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]
then
node_ip="$(whiptail --title "ZelNode ANALIZER/FiXER v2.0" --inputbox "Enter your node IP" 8 72 3>&1 1>&2 2>&3)"
zel_id="$(whiptail --title "ZelNode ANALIZER/FiXER v2.0" --inputbox "Enter your ZEL ID from ZelCore (Apps -> Zel ID) " 8 72 3>&1 1>&2 2>&3)"
