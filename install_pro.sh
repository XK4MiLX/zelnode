...
function start_daemon() {
	...
	if [[ -f /home/$USER/$CONFIG_DIR/debug.log ]]; then
	  error_line=$(egrep -a --color 'Error:' /home/$USER/$CONFIG_DIR/debug.log | tail -1 | sed -E 's/[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}.[0-9]\{2\}.[0-9]\{2\}.[0-9]\{2\}.//')	  
	     if [[ "$error_line" != "" ]]; then	  
...
if [[ "$IMPORT_ZELID" == "0" ]]; then
        ...
        if whiptail --yesno "Are you planning to run Kadena node? Please note that only Nimbus/Stratus nodes are allowed to run it. ( to get reward u still NEED INSTALL KadenaChainWebNode under Apps -> Local Apps section via FluxOS Web UI )" 10 90 3>&1 1>&2 2>&3; then
